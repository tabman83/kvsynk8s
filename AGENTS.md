...
## Distribution (how this repo actually ships)

This repo does NOT follow the generic kubebuilder distribution recipes
(committed `dist/install.yaml`, `helm/v2-alpha` chart regeneration). Both
install methods are produced by `.github/workflows/release.yml`.

### Install manifest (install.yaml)

- Rendered by the release workflow (`kustomize build config/default` with the
  image pinned to the release tag) and attached to the GitHub Release. It is
...
- The chart is hand-maintained, reviewed source at `charts/kvsynk8s/` — it is
  not generated by any kubebuilder plugin. Do NOT run
  `kubebuilder edit --plugins=helm/v2-alpha` (with or without `--force`): it
  would clobber the hand-maintained chart.
- Two machine-managed regions are regenerated by `make helm-sync`: the whole
  of `templates/crds/secretsync-crd.yaml` and the rules between the
  `# BEGIN/END generated rules` markers in
  `templates/rbac/manager-clusterrole.yaml`. Never edit those by hand.
- The release workflow packages the chart (overriding the `0.0.0` placeholder
  version at package time) and publishes it to
  `oci://ghcr.io/tabman83/charts/kvsynk8s`, attaching the `.tgz` to the
  GitHub Release as well. Users install with:
  ```bash
  helm install kvsynk8s oci://ghcr.io/tabman83/charts/kvsynk8s \
    --version X.Y.Z --namespace kvsynk8s --create-namespace
  ```

### Container image

- Built and pushed (multi-arch) by the same release workflow to
  `ghcr.io/tabman83/kvsynk8s`. No manual `docker push` is part of the flow.

See CLAUDE.md "Cutting a release" for the triggers (tag, master merge dev
builds, workflow_dispatch).
