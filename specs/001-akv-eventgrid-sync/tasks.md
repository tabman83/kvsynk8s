...
- [X] T008 [P] Wire operator configuration in cmd/main.go: flags/env for queue URL, reconcile interval (default 4h — was 1h until PR #15), client ID; manager setup with health/ready probes; no leader election (single replica, plan.md)
...
**Goal**: The cluster never drifts permanently: periodic reconciliation (default 4 h), startup convergence, in-cluster drift repair, and vault-side deletion handling.
...
- [X] T023 [US3] Add periodic reconciliation in internal/controller/secretsync_controller.go: RequeueAfter with the configured interval (default 4 h since PR #15) on every successful reconcile; startup convergence needs no extra code (informer sync enqueues all CRs) — assert it in the envtest from T021
...
- [X] T029 [P] Queue integration tests in internal/azure/queue_integration_test.go (build tag `integration`): testcontainers-go starts Azurite; exercise the azqueue-backed QueueSource against it — batch receive of 32, delete, DequeueCount visibility-timeout behavior, and a Base64-encoded Event Grid payload roundtrip per contracts/queue-message.md
- [X] T030 [P] Key Vault reader integration tests in internal/azure/keyvault_integration_test.go (build tag `integration`): testcontainers-go starts Lowkey Vault; exercise the azsecrets-backed SecretReader — GetLatest happy path, new-version-wins, not-found, disabled-secret error mapping. Lowkey Vault's challenge-auth flow worked with the unmodified Go SDK (DisableChallengeResourceVerification only, since the emulator isn't a real *.vault.azure.net host) — no HTTPS-stub fallback was needed; see research.md R10 for the one real finding this surfaced (disabled-secret detection had to move off status-code alone)
- [X] T031 Add Makefile targets and tooling: `test-integration` (runs `go test -tags integration ./internal/azure/...`, requires Docker), `test-e2e` (creates/uses a kind cluster, builds and loads the operator image), pinned tool versions for envtest/golangci-lint in the Makefile (envtest derived from go.mod, golangci-lint pinned explicitly; kind is deliberately unpinned — `KIND ?= kind` uses whatever is on PATH, and CI installs the latest)
- [X] T032 E2E suite in test/e2e/secretsync_test.go against kind: deploys CRD + operator via config/default with env pointed at Azurite and Lowkey Vault on the kind network, and asserts the full loop — SecretSync created -> Secret appears with the correct value; SecretNewVersionCreated injected into the queue -> Secret updated within SC-001's bound; managed Secret deleted in-cluster -> recreated; SecretSync deleted -> managed Secret removed; pre-existing unmanaged Secret -> TargetConflict and left untouched; a planted sentinel never appears in logs, status or events (SC-004). **Done and running by default**: `make test-e2e` is 8 of 8 specs, 0 skipped. This was opt-in behind KVSYNK8S_E2E_EMULATORS=1 for a long time because of what was believed to be a cluster-DNS flake; that diagnosis was wrong — authstub was dying on startup, unable to read its bind-mounted TLS key as a non-root uid. Root cause, the fix, and the two further defects that un-gating exposed (a missing go test timeout and a finalizer deadlock in teardown) are written up in research.md R10.
- [X] T033 CI workflow: `.github/workflows/test-integration.yml` added (new file, matching this repo's one-workflow-per-concern convention rather than a single ci.yml) runs `make test-integration` on every PR; the existing `test.yml`/`lint.yml`/`test-e2e.yml` continue to run `make test`/lint/`make test-e2e` unchanged and still pass — constitution IV's full suite (lint, test, test-integration, test-e2e) runs on every PR; pinned action versions match the existing SHA+version-comment style
...
- [X] T034 [P] Harden the Dockerfile: distroless static base, non-root, multi-arch (amd64/arm64) via `make docker-buildx`
...
- [X] T036 Release pipeline in .github/workflows/release.yml. It now has three triggers: pushing a `v[0-9]*` tag, a merge to `master` (dev build: image and chart pushed as `<next patch>-dev.<run number>`, no tag, no GitHub Release, `:latest` untouched), and `workflow_dispatch` with a `version` input (creates the tag at the end of the run). A stable release runs the full test suite, builds and pushes the multi-arch image to ghcr.io/tabman83/kvsynk8s tagged `vX.Y.Z` and `latest`, renders a single install manifest (`kustomize build config/default` with the image pinned to the tag) as install.yaml, publishes the chart to oci://ghcr.io/tabman83/charts/kvsynk8s, and creates a GitHub Release with install.yaml and the chart .tgz attached and generated notes; all action versions pinned (constitution: pinned dependencies)
- [X] T037 Update CLAUDE.md with real build/lint/test commands (make build/test/lint/test-integration/test-e2e, single-test invocation), the actual architecture, and the "PR CI babysitting" rule from Notes below (constitution: Development Workflow — required now that code exists)
