...
- [X] T031 Add Makefile targets and tooling: `test-integration` (runs `go test -tags integration ./internal/azure/...`, requires Docker), `test-e2e` (creates/uses a kind cluster, builds and loads the operator image), pinned tool versions for kind/envtest/golangci-lint in the Makefile
...
- [X] T036 Release pipeline in .github/workflows/release.yml: on pushing a `v*` tag — run the full test suite, build and push the multi-arch image to ghcr.io/tabman83/kvsynk8s tagged `vX.Y.Z` and `latest`, render a single install manifest (`kustomize build config/default` with the image pinned to the tag) as install.yaml, and create a GitHub Release with install.yaml attached and generated notes; pin all action versions (constitution: pinned dependencies)
