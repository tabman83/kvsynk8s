...
- [X] T024 [US3] Manual post-merge validation: **substantially done, on the first dev build rather than a tag.** Merging #19 published `0.1.0-dev.1` through the real pipeline on 2026-08-25. Verified against the live registry: both packages exist and are **public** (an anonymous `helm install oci://ghcr.io/tabman83/charts/kvsynk8s --version 0.1.0-dev.1 --dry-run`, with an empty registry config, pulled and rendered the chart); the image published as `ghcr.io/tabman83/kvsynk8s:v0.1.0-dev.1`; `:latest` was correctly *not* moved and no GitHub Release was created, both being prerelease/dev behaviour. The GHCR visibility worry recorded here earlier was wrong — see contracts/release-artifacts.md. **Done.** The remaining part, the one a dev build cannot exercise, was confirmed on the `v0.1.0` release: the GitHub Release carries both assets (SC-003, FR-014) — `install.yaml` (11271 bytes, `sha256:8083757ff50682e46ac6e4c34336ac5d47aa93fc69713acfa84f39534a4450c4`) and `kvsynk8s-0.1.0.tgz` (9601 bytes, `sha256:8df11c00c07673d887ca0949d5cc57d8d66cca00c472f0254784909e11b14f4b`), both uploaded by the same run at 2026-08-25T06:17:40Z, on a release that is neither a draft nor a prerelease and is tagged Latest.
